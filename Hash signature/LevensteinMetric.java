...
public class LevensteinMetric extends Metric {
	...
	/**
	 * {@inheritDoc} Levenshtein distance is calculated for the asymptotic time O((max + 1) * min(first.length(),
	 * second.length()))
	 */
	@Override
	public int getDistance(CharSequence first, CharSequence second, int max) {
		...
		for (int i = 1; i <= secondLength; i++) {
			...
			// We calculate only the diagonal strip width 2 * (max + 1)
			int from = Math.max(i - max - 1, 1);
			int to = Math.min(i + max + 1, firstLength);
			for (int j = from; j <= to; j++) {
// Calculate the minimum price of the transition to the current state from the previous ones among deletion, insertion and
// replacements respectively.
	...
	/**
	 * {@inheritDoc} Levenshtein prefix distance is calculated in asymptotic time O((max + 1) *
